...
export default function Accordion({ item, index }: { item: any; index: number }) {
  ...
  return (
    <>
      ...
      <h5
        className={`${
          moCateOpen[index] ? 'before:rotate-0 before:opacity-0 ' : 'before:rotate-90 before:opacity-100'
      ...
      >
        {item.list.map((list: any, subIndex: number) => (
          <li key={subIndex} className={`${moCateOpen[index] ? 'h-full block' : 'h-0 hidden'} py-3 text-lg text-black px-7`}>
